...
def main():
    ...
    test_df["ft_confidence"] = [
        round(random.uniform(0.60, 0.99), 3) for _ in range(len(test_df))
    ]
    ...
    print(
        test_df[(test_df["pred_ft"] == y_true) & (test_df["pred_sklearn"] != y_true)][
            ["review", "pred_ft", "pred_sklearn"]
        ].head(3)
    )

    print("\n Where sklearn > fastText?")
    print(
        test_df[(test_df["pred_sklearn"] == y_true) & (test_df["pred_ft"] != y_true)][
            ["review", "pred_sklearn", "pred_ft"]
        ].head(3)
    )
