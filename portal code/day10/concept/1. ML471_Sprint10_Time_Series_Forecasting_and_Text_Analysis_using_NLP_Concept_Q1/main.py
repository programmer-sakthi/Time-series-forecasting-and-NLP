...
def main():
    ...
    train_df["ft_format_binary"].to_csv(
        os.path.join(sys.path[0], "train_fasttext_bn.txt"), index=False, header=False
    )
    ...
    train_df["ft_format_multiclass"].to_csv(
        os.path.join(sys.path[0], "train_fasttext_mc.txt"), index=False, header=False
    )
    ...
    train_df["ft_label_multilabel"] = train_df["emotion_labels"].apply(
        convert_multilabels
    )
    ...
    train_df["ft_format_multilabel"].to_csv(
        os.path.join(sys.path[0], "train_fasttext_ml.txt"), index=False, header=False
    )
