...
def main():
    ...
    multi_pipeline = Pipeline(
        [("tfidf", TfidfVectorizer(max_features=2000)), ("clf", MultinomialNB())]
    )
    ...
    # ============================
    test_df["agree_ft_sk"] = test_df["pred_ft"] == test_df["pred_sklearn"].map(
        {v: k for k, v in label_mapping.items()}
    )
    test_df["agree_ft_gen"] = test_df["pred_ft"] == test_df["pred_genai"]
    test_df["agree_sk_gen"] = (
        test_df["pred_sklearn"].map({v: k for k, v in label_mapping.items()})
        == test_df["pred_genai"]
    )
    ...
    # ============================
    test_df["ft_pred_raw"] = test_df["clean_text"].apply(
        lambda x: (fasttext_predict_mock(x), random.random())
    )
