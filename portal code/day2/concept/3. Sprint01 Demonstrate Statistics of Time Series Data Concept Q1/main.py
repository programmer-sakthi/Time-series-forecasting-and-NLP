...
df["Consumption"] = np.where(
    df["Consumption"] < lower_bound, df["Consumption"].mean(), df["Consumption"]
)

df["Consumption"] = np.where(
    df["Consumption"] > upper_bound, df["Consumption"].mean(), df["Consumption"]
)
...
multiplicative = seasonal_decompose(
    df["Consumption"], model="multiplicative", period=12
)
